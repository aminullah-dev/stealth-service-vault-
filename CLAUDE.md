...
- `ios/` — iOS app (SwiftUI, Firebase). `SafeBeautyCore` is a local Swift
  package holding the logic ported from the server — slot maths, PinHasher,
  PhoneUtils, the Firestore models — with tests that assert parity against it.
  The `.xcodeproj` is generated by XcodeGen from `project.yml` and is not
  committed; run `xcodegen generate` after a fresh clone.
- `functions/` — Firebase Cloud Functions v2 (Node 22, region `us-central1`).
  `index.js` is now only the export list — the code lives in `domains/*.js`
  (identity, payments, bookings, content, discovery, admin, maintenance,
  notifications) with the pure, tested parts in `lib/*.js`. Project id
  `safebeauty`, Blaze plan.
...
- **Verify before commit**: `npm test` in `functions/` — it runs `eslint .`
  ...
  brace/paren balance and 4× string counts for Kotlin/AppStrings.
...
- **Marketing/social images** (Instagram posts, stories, banners): deliver
  ...
  render with `generate.py` in the marketing folder rather than hand-building
  a second renderer — there are already two and they disagree on typography
  and output filenames; post 1080×1350, story
  1080×1920.
...
- **Check `main` before trusting a fresh clone** — `git log --oneline -1 origin/main`.
  It was brought current on 2026-09-04 and the working branch has moved on since,
  so the two are not level. It had been ninety-two commits behind before that:
