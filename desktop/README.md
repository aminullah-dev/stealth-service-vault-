# SafeBeauty Admin — Desktop App

A native desktop wrapper (Electron) around the admin console at
`https://safebeauty.web.app/admin`. It opens the console in its own window so it
feels like a real Mac/Windows app, and always shows the latest deployed version.

## Prerequisites
- Node.js (you already have it — it's what runs the Cloud Functions).

## First-time setup
```bash
cd desktop
npm install
```

## Run it (without building an installer)
```bash
npm start
```

## Build a Mac installer (.dmg)  — run this ON a Mac
```bash
npm run dist:mac
```
The `.dmg` lands in `desktop/dist/`. Double-click it, drag **SafeBeauty Admin**
to Applications, done.

> The app is **not code-signed**, so the first time macOS will say
> "unidentified developer". Right-click the app → **Open** → **Open** to allow it
> (only needed once). Proper signing needs a paid Apple Developer account.

## Build a Windows installer (.exe) — run this ON Windows
```bash
npm run dist:win
```
The `.exe` (NSIS installer) lands in `desktop/dist/`.

> Building a Windows `.exe` must be done on a Windows machine (or via CI). You
> can't reliably cross-build it from a Mac. If you don't have a Windows PC, ask
> and we can add a GitHub Actions workflow that builds the `.exe` in the cloud.

## Notes
- Login uses the same admin phone + password as the Android app.
- Since it loads the hosted console, you don't need to rebuild the desktop app
  when the console changes — just `firebase deploy --only hosting` and reopen.
